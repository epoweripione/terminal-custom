...
GITHUB_CHECK_CURL_OPTION=(--connect-timeout 5)
GITHUB_DOWNLOAD_CURL_OPTION=(--connect-timeout 5)
if [[ -z "$GITHUB_NOT_USE_PROXY" ]]; then
    ...
    [[ -n "${GITHUB_MIRROR_USE_CNPMJS}" ]] && \
        GITHUB_DOWNLOAD_URL="https://github.com.cnpmjs.org" && \
        GITHUB_DOWNLOAD_CURL_OPTION=("${GITHUB_DOWNLOAD_CURL_OPTION[@]}" --noproxy "*")

    [[ -n "${GITHUB_MIRROR_USE_FASTGIT}" ]] && \
        GITHUB_DOWNLOAD_URL="https://download.fastgit.org" && \
        GITHUB_DOWNLOAD_CURL_OPTION=("${GITHUB_DOWNLOAD_CURL_OPTION[@]}" --noproxy "*")
fi

export GITHUB_CHECK_CURL_OPTION=${GITHUB_CHECK_CURL_OPTION:-""}
export GITHUB_DOWNLOAD_CURL_OPTION=${GITHUB_DOWNLOAD_CURL_OPTION:-""}
