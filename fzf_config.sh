...
export FZF_DEFAULT_OPTS="--ansi --multi --height=50%"
...
# fzf alias
alias fzf-cat='${FZF_REAL_COMMAND:-fzf} --layout=reverse --preview-window=right,70%,wrap \
...
if [[ -x "$(command -v bat)" ]]; then
    ...
    alias fzf-bat='${FZF_REAL_COMMAND:-fzf} --layout=reverse --preview-window=right,70%,wrap \
        --bind="ctrl-j:preview-page-down,ctrl-k:preview-page-up,alt-j:preview-bottom,alt-k:preview-top" \
